...
Из интерфейса можно выполнять команды storcli трёх уровней:
...
| Кнопка | Действие | Объект | Команда storcli |
|--------|----------|--------|-----------------|
| ➕ Создать | Создать RAID | Controller | `/cx add vd type=raidX drives=...` |
...
#### 🔴 Опасные (требуют ввода проверочной фразы)

| Кнопка | Действие | Объект | Команда storcli |
|--------|----------|--------|-----------------|
| 🗑️ | Удалить VD | VD | `/cx/vX del force` |
| 🔌 | Force Offline | PD | `/cx/ex/sx set offline` |
| ⚡ | Force Online | PD | `/cx/ex/sx set online` |

> ⚠️ Операционные команды показывают модальное окно подтверждения. Опасные команды требуют ручного ввода названия действия для исключения случайного нажатия. Все действия логируются.

## 🛠 Требования

Для работы приложения необходимо:

### На управляющей машине (где запущен Python)
- Python 3.9+
- Доступ к целевым серверам по SSH

### На целевых серверах (хостах)
- **StorCLI**: Утилита должна быть установлена. По умолчанию приложение ищет её по пути `/opt/lsi/storcli/storcli`, но путь можно переопределить в настройках хоста.
- **SSH server**: Должен быть запущен и настроен доступ для пользователя (root или с правами sudo).
- Поддержка вывода в формате JSON (современные версии StorCLI).
